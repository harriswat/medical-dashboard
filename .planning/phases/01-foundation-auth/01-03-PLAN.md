...
must_haves:
  truths:
    ...
    - "When offline, user can view content but actions are disabled (read-only per user decision)"
...
<objective>
Build the mobile-first app shell with warm visual design, bottom navigation (Today/History tabs), real-time Supabase subscriptions, online/offline detection, and sync status indicator. This transforms the auth skeleton from Plans 01-02 into a polished, usable shell ready for Phase 2 content.
...
<task type="auto">
  ...
  <action>
    ...
    4. Update `src/app/(dashboard)/layout.tsx`:
       - Server component that checks auth (keep from Plan 02)
...
<task type="auto">
  ...
  <action>
    ...
    6. Create `src/components/offline/OfflineBanner.tsx` (client component):
       ...
       - This message reflects the locked decision: **read-only when offline** (no queued mutations, no offline writes)
  ...
  <done>
    Sync status indicator shows "Synced" / "Syncing..." / "Offline" in the header. Offline banner appears when network is lost with warm amber styling and read-only messaging. Real-time subscription hook is working and properly cleans up. Online status detection combines browser events with periodic health checks. All hooks export cleanly for reuse in Phase 2+.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    6. Test offline: put phone in airplane mode (or use DevTools offline)
       ...
       - You should still be able to view the page (read-only -- no action buttons active)
  ...
  <done>User has approved the Phase 1 experience -- warm design, auth, navigation, sync status, and offline read-only mode all verified on mobile.</done>
...
- Offline banner appears when disconnected, content still viewable (read-only)
